...
namespace Dragonfire.Logging.Generator
{
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(services.Collect(), (spc, models) =>
            {
                try
                {
                    ...
                    {
                        spc.AddSource(
                            "DragonfireGeneratedScopeState.g.cs",
                            SourceText.From(ScopeStateEmitter.Emit(), Encoding.UTF8));
                    }
                    catch (Exception ex)
                    {
                        var msg = $"Failed to emit scope state helper: {ex}";
                        Console.Error.WriteLine($"[Dragonfire.Logging.Generator] {msg}");
                        spc.ReportDiagnostic(Diagnostic.Create(s_generatorError, Location.None, msg));
                    }

                    // Shared: DI registration extension
                    try
                    {
                        spc.AddSource(
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Scope state wrapper emitter
    //
    // Emits __DragonfireScopeState — a Dictionary wrapper that:
    //   • Implements IEnumerable<KVP> so every entry becomes a customDimension
    //     in Application Insights / Seq / OpenTelemetry
    //   • Overrides ToString() with a human-readable label so the local
    //     JsonConsoleFormatter shows a clean "Message" instead of the type name
    // ─────────────────────────────────────────────────────────────────────────────

    internal static class ScopeStateEmitter
    {
        public static string Emit()
        {
            var sb = new StringBuilder();
            ...
            sb.AppendLine("{");
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// Scope state bag used by all Dragonfire logging proxies.");
            sb.AppendLine("    /// Implements <c>IEnumerable&lt;KeyValuePair&gt;</c> so structured log providers");
            sb.AppendLine("    /// (Application Insights, Seq, OpenTelemetry) read each entry as a separate");
            sb.AppendLine("    /// <c>customDimension</c>. <c>ToString()</c> returns the baked method label so");
            sb.AppendLine("    /// the local JSON console formatter shows a readable <c>Message</c> field.");
            sb.AppendLine("    /// </summary>");
            sb.AppendLine("    [global::System.CodeDom.Compiler.GeneratedCode(\"Dragonfire.Logging.Generator\", \"1.0.0\")]");
            sb.AppendLine("    [global::System.Diagnostics.DebuggerNonUserCode]");
            ...
            sb.AppendLine();
            sb.AppendLine("        internal object? this[string key]");
            sb.AppendLine("        {");
            sb.AppendLine("            get => _data.TryGetValue(key, out var v) ? v : null;");
            sb.AppendLine("            set => _data[key] = value;");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        /// <summary>Returns the human-readable method label baked at compile time.</summary>");
            sb.AppendLine("        public override string ToString() => _message;");
            sb.AppendLine();
            sb.AppendLine("        public global::System.Collections.Generic.IEnumerator<global::System.Collections.Generic.KeyValuePair<string, object?>> GetEnumerator()");
            sb.AppendLine("            => _data.GetEnumerator();");
            sb.AppendLine();
    ...
    // Generated AddDragonfireGeneratedLogging() only injects ILogger<TClass> —
    // no Dragonfire runtime service lookups.
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    internal static class RegistrationEmitter
    {
        public static string Emit(ImmutableArray<ServiceModel> models)
        {
            ...
            sb.AppendLine("{");
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// DI registration for all compile-time logging proxies.");
            sb.AppendLine("    /// Each proxy depends only on <c>ILogger&lt;T&gt;</c> — no Dragonfire runtime services.");
            sb.AppendLine("    /// Call <c>services.AddDragonfireGeneratedLogging()</c> after registering your services.");
            sb.AppendLine("    /// </summary>");
            sb.AppendLine("    [global::System.CodeDom.Compiler.GeneratedCode(\"Dragonfire.Logging.Generator\", \"1.0.0\")]");
